...
> Durum: **Onaylandı — Faz 0 yürürlükte.** Alınan kararlar için §8'e bakınız.
> Tarih: 2026-08-14
...
| B3  | ACL otoritesi                         | §6.2 "iki ayrı gerçeklik üretme" diyor ama Samba/POSIX ile DEPSIS modeli birebir örtüşmez                                                                 | **DEPSIS ACL tek kaynak.** ZFS `acltype=nfsv4` + `xattr=sa`, Samba `vfs objects = acl_xattr`. POSIX draft ACL kullanılmaz (deny semantiği yok, maske davranışı sızıntı üretir). → ADR-0004                                          |
...
```bash
sudo bash tools/poc/zfs-pool.sh          # P0-A: mirror, snapshot, send/receive, scrub
sudo bash tools/poc/samba-acl.sh         # P0-B: acltype=nfsv4 + smbcacls doğrulaması
testparm -s /etc/samba/smb.conf          # config publish öncesi zorunlu kapı
psql -v ON_ERROR_STOP=1 -f tools/poc/pg-rls.sql   # P0-C: cross-tenant SELECT = 0 satır
sudo cargo run -p depsis-fs-events       # P0-D: SMB yazması → olay gecikmesi ölçümü
pnpm exec tsx tools/poc/agent-smoke.ts   # P0-E: serbest komut reddi + audit kaydı
```
